...
func test_ui_scenes_are_editor_authored_for_360_viewport() -> void:
	...
	var room_decor_item_row_scene := FileAccess.get_file_as_string("res://scenes/ui/RoomDecorItemRow.tscn")
	var room_decor_item_row_source := FileAccess.get_file_as_string("res://scripts/ui/RoomDecorItemRow.gd")
	var decor_catalog_scene := FileAccess.get_file_as_string("res://scenes/ui/DecorCatalogContent.tscn")
	var decor_catalog_source := FileAccess.get_file_as_string("res://scripts/ui/DecorCatalogContent.gd")
	var space_decor_panel_scene := FileAccess.get_file_as_string("res://scenes/ui/SpaceDecorPanel.tscn")
	var space_decor_panel_source := FileAccess.get_file_as_string("res://scripts/ui/SpaceDecorPanel.gd")
	...
	assert_true(room_decor_item_row_scene.contains("[node name=\"Card\" type=\"VBoxContainer\""), "RoomDecorItemRow should use an editor-authored card layout")
	assert_true(room_decor_item_row_scene.contains("[node name=\"PreviewFrame\" type=\"PanelContainer\""), "RoomDecorItemRow should expose a preview frame")
	assert_true(room_decor_item_row_scene.contains("ApplyButton"), "RoomDecorItemRow should expose its apply button")
	assert_true(room_decor_item_row_scene.contains("[node name=\"PriceButtonTemplate\" type=\"Label\""), "RoomDecorItemRow should keep price templates as scene labels")
	assert_true(room_decor_item_row_scene.contains("[node name=\"AppliedButtonText\" type=\"Label\""), "RoomDecorItemRow should keep applied-state copy in the scene")
	assert_true(room_decor_item_row_scene.contains("[node name=\"ApplyInsufficientText\" type=\"Label\""), "RoomDecorItemRow should keep disabled copy in the scene")
	assert_false(room_decor_item_row_source.contains("@export"), "RoomDecorItemRow should not require script exports for editor-authored text templates")
	assert_false(room_decor_item_row_source.contains("已应用"), "RoomDecorItemRow should not hard-code applied-state copy in script")
	assert_false(room_decor_item_row_source.contains("金币不足"), "RoomDecorItemRow should not hard-code disabled copy in script")
	...
	assert_true(tab_button_source.contains("func setup_toggle"), "PanelTabButton should support selected toggle filters without disabling the button")
	...
	assert_true(decor_catalog_scene.contains("DoorDecorFilter"), "DecorCatalogContent should expose a door decor filter")
	assert_true(decor_catalog_scene.contains("RoofDecorFilter"), "DecorCatalogContent should expose a roof decor filter")
	assert_true(decor_catalog_scene.contains("DecorListRoot"), "DecorCatalogContent should expose a single filtered decor list root")
	assert_true(decor_catalog_scene.contains("metadata/tab_id = \"wallpaper\""), "DecorCatalogContent wallpaper filter should be scene-keyed")
	assert_true(decor_catalog_scene.contains("metadata/tab_id = \"wall\""), "DecorCatalogContent wall filter should be scene-keyed")
	assert_true(decor_catalog_scene.contains("metadata/tab_id = \"door\""), "DecorCatalogContent door filter should be scene-keyed")
	assert_true(decor_catalog_scene.contains("metadata/tab_id = \"roof\""), "DecorCatalogContent roof filter should be scene-keyed")
	...
	assert_true(room_panel_scene.contains("metadata/tab_id = \"decor\""), "RoomPanel decor tab id should be scene metadata")
	...
	assert_true(shop_panel_scene.contains("FilterRoot"), "FurnitureShopPanel should expose a scene-authored filter area")
	assert_true(shop_panel_scene.contains("[node name=\"CategoryDropdown\" type=\"OptionButton\""), "FurnitureShopPanel should expose category filters as a dropdown")
	assert_true(shop_panel_scene.contains("[node name=\"SortDropdown\" type=\"OptionButton\""), "FurnitureShopPanel should expose sort filters as a dropdown")
	assert_false(shop_panel_scene.contains("CategoryFilterGrid"), "FurnitureShopPanel should not keep the wide category tab grid")
	assert_false(shop_panel_scene.contains("SortFilterGrid"), "FurnitureShopPanel should not keep the wide sort tab grid")
	assert_true(shop_panel_scene.contains("QuickFilterGrid"), "FurnitureShopPanel should expose quick filters")
	assert_true(shop_panel_scene.contains("ResultLabel"), "FurnitureShopPanel should expose filtered result counts")
	assert_true(shop_panel_scene.contains("EmptyStateLabel"), "FurnitureShopPanel should expose an empty filter state")
	assert_true(shop_panel_scene.contains("[node name=\"CategoryOptions\" type=\"Control\""), "FurnitureShopPanel should keep category dropdown options as scene templates")
	assert_true(shop_panel_scene.contains("[node name=\"SortOptions\" type=\"Control\""), "FurnitureShopPanel should keep sort dropdown options as scene templates")
	assert_true(shop_panel_scene.contains("metadata/tab_id = \"__all__\""), "FurnitureShopPanel should expose an all-category filter")
	for category in ["床", "桌椅", "娱乐", "收纳", "厨房", "装饰", "灯具", "墙面"]:
		assert_true(shop_panel_scene.contains("metadata/tab_id = \"%s\"" % category), "FurnitureShopPanel should expose %s category filter" % category)
	for sort_id in ["default", "price_asc", "price_desc", "score", "comfort", "entertainment", "hygiene", "food"]:
		assert_true(shop_panel_scene.contains("metadata/tab_id = \"%s\"" % sort_id), "FurnitureShopPanel should expose %s sort filter" % sort_id)
	for filter_id in ["affordable", "floor_item", "wall_item", "interactive", "rotatable"]:
		assert_true(shop_panel_scene.contains("metadata/tab_id = \"%s\"" % filter_id), "FurnitureShopPanel should expose %s quick filter" % filter_id)
	...
	assert_true(shop_panel_scene.contains("[node name=\"ResultCountTemplate\" type=\"Label\""), "FurnitureShopPanel should keep result count templates as scene labels")
	...
	assert_false(shop_panel_source.contains("没有符合条件"), "FurnitureShopPanel should not hard-code empty state copy in script")
	assert_false(shop_panel_source.contains("低价优先"), "FurnitureShopPanel should not hard-code dropdown option copy in script")
	assert_true(shop_panel_source.contains("_populate_dropdown_from_templates"), "FurnitureShopPanel should populate dropdowns from scene-authored option templates")
	assert_true(shop_panel_source.contains("GameEvents.coins_changed.connect(_on_coins_changed)"), "FurnitureShopPanel should refresh affordability state when coins change")
	assert_true(shop_panel_source.contains("ConfigManager.get_furniture_score(item)"), "FurnitureShopPanel should sort by config-driven furniture score")
