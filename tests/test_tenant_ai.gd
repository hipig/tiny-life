...
func test_tenant_uses_character_body_click_area_and_strict_animations() -> void:
	...
	assert_true(ai_source.contains("tenant.visible = false"), "TenantAI should hide tenants only after they are inside a closed source elevator")
	assert_true(ai_source.contains("tenant.visible = true"), "TenantAI should show tenants again when the target elevator opens")
...
func test_tenant_presence_state_is_saved_and_normalized() -> void:
	...
	assert_eq(float(ai_config.get("elevator_animation_seconds", 0.0)), 0.36, "Tenant elevator doors should use the slower configured open/close duration")
