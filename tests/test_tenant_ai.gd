...
func test_tenant_uses_character_body_click_area_and_strict_animations() -> void:
	...
	assert_true(need_bubble_scene.contains("metadata/behavior_key = \"sit\""), "NeedBubble should expose a sit behavior icon through scene metadata")
	...
	assert_true(ai_source.contains("GameState.TENANT_PRESENCE_LEAVING:\n\t\t\t_resume_leaving_route()"), "Leaving tenants should resume through their dedicated route branch")
	assert_true(ai_source.contains("GameState.TENANT_PRESENCE_RETURNING:\n\t\t\t_resume_returning_route()"), "Returning tenants should resume through their dedicated route branch")
	assert_false(ai_source.contains("GameState.TENANT_PRESENCE_LEAVING, GameState.TENANT_PRESENCE_RETURNING"), "Leaving and returning tenants should not share the same restore branch anymore")
	...
	assert_true(ai_source.contains("func _resume_home_state"), "TenantAI should expose a dedicated HOME restore helper")
	assert_true(ai_source.contains("func _resume_leaving_route"), "TenantAI should expose a dedicated LEAVING restore helper")
	assert_true(ai_source.contains("func _resume_away_state"), "TenantAI should expose a dedicated AWAY restore helper")
	assert_true(ai_source.contains("func _resume_returning_route"), "TenantAI should expose a dedicated RETURNING restore helper")
	assert_true(ai_source.contains("func _ensure_room_resume_position"), "TenantAI should recover room-layer leaving tenants back to a stable local room position before exiting")
	assert_true(ai_source.contains("func _recover_room_position"), "TenantAI should have a dedicated local-room recovery helper")
	assert_true(ai_source.contains("func _tenant_in_world_layer"), "TenantAI should distinguish room-layer leaving tenants from world-layer route resumes")
	assert_true(ai_source.contains("func _recover_from_origin_position_if_needed"), "TenantAI should recover route tenants that get reset to the origin")
	assert_true(ai_source.contains("func _require_world_anchor"), "TenantAI should validate world-route anchors before walking toward them")
	assert_true(ai_source.contains("func _recover_missing_route_anchor"), "TenantAI should fail closed to a stable room spawn when anchors are missing")
	assert_true(ai_source.contains("if view != null and _tenant_in_world_layer(view):\n\t\tif not _ensure_route_resume_position(GameState.TENANT_PRESENCE_LEAVING, \"resume leaving route\")"), "Leaving restore should only use world-route resume positions when the tenant is already on the shared world layer")
	assert_true(ai_source.contains("elif not _ensure_room_resume_position(\"resume leaving route\")"), "Leaving restore should keep room-layer tenants in local room coordinates until they actually exit")
	...
	assert_false(ai_source.contains("exit_position = tenant.position"), "TenantAI should not fall back to the current tenant position when the public exit anchor is missing")
	assert_false(ai_source.contains("return tenant.position"), "TenantAI should not use the current tenant position as a route-anchor fallback")
	...
	assert_true(ai_source.contains("var resumed_from_world := _tenant_in_world_layer(view)"), "Leaving route should branch between full in-room exits and resumed world-layer routes")
	assert_true(ai_source.contains("if not resumed_from_world:\n\t\tif not await _move_to_position(TenantRoomLocator.room_door_inside_position(_room()), \"walk to room door\")"), "Fresh leaving routes should still walk to the local room door before world promotion")
	assert_true(ai_source.contains("if not resumed_from_world:\n\t\tawait _play_room_door(view, false)"), "Only fresh leaving routes should close the room door after walking out")
	...
	assert_true(ai_source.contains("interaction.get(\"behavior\""), "TenantAI should use canonical furniture interaction behavior keys")
	assert_true(ai_source.contains("interaction.get(\"satisfaction_delta\""), "TenantAI should apply configured interaction satisfaction deltas")
	assert_true(ai_source.contains("furniture_interaction_started.emit"), "TenantAI should emit furniture interaction start for visual feedback")
	assert_true(ai_source.contains("furniture_interaction_finished.emit"), "TenantAI should emit furniture interaction finish for visual feedback")
	assert_true(ai_source.contains("tenant_furniture_reaction_requested"), "TenantAI should react to newly purchased furniture events")
...
func test_tenant_room_locator_uses_continuous_floor_line_positions() -> void:
	...
	assert_true(locator_source.contains("orientation"), "Furniture use positions should honor the instance orientation")
...
func test_tenant_behavior_state_uses_keys_with_alias_config() -> void:
	...
	assert_true(alias_config.contains("\"坐下\": \"sit\""), "Behavior aliases should migrate sit display labels to the sit behavior key")
	assert_true(config_source.contains("\"sit\": true"), "ConfigManager should allow the sit behavior key")
	assert_true(game_state_source.contains("\"sit\": true"), "GameState should allow the sit behavior key")
	assert_true(tenant_panel_scene.contains("metadata/behavior_key = \"sit\""), "TenantPanel should expose a sit behavior label through scene metadata")
	...
	assert_true(game_state_source.contains("_normalize_tenant_behavior"), "Observed interactions should normalize canonical behavior keys directly")
	assert_false(game_state_source.contains("_need_to_behavior_key"), "GameState should not keep a need-to-behavior mapping after furniture interactions declare behavior keys")
	assert_true(game_state_source.contains("func observe_tenant_behavior(tenant_id: String, behavior_key: String, satisfaction_delta := 1)"), "Observed furniture interactions should store behavior keys and configurable satisfaction deltas")
	assert_true(game_state_source.contains("func react_to_new_furniture"), "GameState should expose new furniture purchase reactions")
	assert_true(game_state_source.contains("_tenant_prefers_furniture"), "GameState should check tenant favorite tags for stronger furniture reactions")
	assert_true(events_source.contains("signal tenant_furniture_reaction_requested"), "Purchasing furniture should request tenant reactions through GameEvents")
	assert_true(events_source.contains("signal furniture_interaction_started"), "Furniture interactions should expose a start event")
	assert_true(events_source.contains("signal furniture_interaction_finished"), "Furniture interactions should expose a finish event")
