...
func test_building_traffic_scenes_support_short_away_routes() -> void:
	...
	assert_true(public_area_source.contains("apply_visual_theme(door_theme)"), "Public entrance doors should apply configured door decor themes")
	assert_true(floor_source.contains("ConfigManager.door_theme_from_decor_state(public_area_state)"), "Public area entrance doors should receive door decor from the unified target state")
	...
	assert_false(floor_source.contains("roof_theme_for_floor"), "Floor should not own per-floor roof theme rendering")
	assert_false(floor_source.contains("show_roof_on_top_room"), "Floor should not decide roof visibility per room segment")
	assert_true(service_source.contains("edge_sides, body_sides, \"\")"), "Central elevator halls should render without left/right door wall cutouts")
	...
	assert_true(apartment_source.contains("@onready var apartment_roof = $ApartmentRoof"), "ApartmentBuilding should own one building-level roof node")
	assert_true(apartment_source.contains("GameState.space_decor_target(ConfigManager.TARGET_ROOF"), "ApartmentBuilding should bind the roof to the unified decor target")
	assert_true(apartment_source.contains("ConfigManager.apartment_roof_theme_for_style(roof_style_id)"), "ApartmentBuilding should render the selected runtime apartment roof theme")
	...
	assert_true(building_view_scene.contains("LeftOffscreenMarker"), "BuildingView should expose a left offscreen marker for completed exit routes")
	...
	assert_false(building_view_source.contains("TenantAI.reset_startup_entry_session()"), "BuildingView should not reset removed startup-entry tracking")
	...
	assert_false(building_view_source.contains("startup_entry_active := TenantAI.is_startup_entry_active"), "BuildingView should not preserve removed startup entry tenants during HOME refreshes")
	assert_true(building_view_source.contains("func _ensure_tenant_views"), "BuildingView should synchronize all tenant visual instances through one owner path")
	assert_true(building_view_source.contains("func _tenant_views_by_id"), "BuildingView should group tenant views by tenant_id before creating new instances")
...
func test_building_outer_wall_edges_follow_floor_rules() -> void:
	...
	assert_true(room_source.contains("anchor_pos"), "Furniture should store continuous room anchors")
	assert_true(room_source.contains("bounds_rect_for_anchor"), "Furniture should position visuals from continuous anchor bounds")
...
func test_floor_and_room_visuals_use_building_atlases() -> void:
	...
	assert_true(floor_source.contains("@onready var left_room = $LeftContent/LeftRoom"), "Floor should bind the preplaced left room node directly")
	assert_true(floor_source.contains("@onready var right_room = $RightContent/RightRoom"), "Floor should bind the preplaced right room node directly")
	...
	assert_true(room_source.contains("world_position_to_placement_anchor"), "Room should translate scene taps through continuous furniture anchors")
	assert_true(room_source.contains("normalize_placement_anchor"), "Room should normalize anchors against floor and wall placement areas")
	...
	assert_true(room_source.contains("ConfigManager.tile_theme_from_decor_state(room)"), "Room should render configured wallpaper and wall themes through unified decor state")
	assert_true(room_source.contains("ConfigManager.door_theme_from_decor_state(room)"), "Room should pass configured door themes to RoomShell through unified decor state")
	...
	assert_true(room_source.contains("func ensure_home_tenant_view"), "Room should expose a targeted HOME tenant view binder")
	...
	assert_false(room_source.contains("for tenant_id in GameState.tenants.keys()"), "Room should not create tenant views independently of BuildingView ownership")
...
func test_saved_room_layouts_are_normalized_to_current_config() -> void:
	...
	assert_true(state_source.contains("\"public_area_decor\""), "Saves should preserve public area decor states")
	assert_true(state_source.contains("\"apartment_decor\""), "Saves should preserve service core and roof decor states")
	assert_true(state_source.contains("func apply_space_decor"), "GameState should expose unified decor application")
	assert_true(state_source.contains("func apply_room_decor"), "GameState should expose per-room decor application")
	assert_true(state_source.contains("GameEvents.decor_target_changed.emit"), "Unified decor application should emit a target-level change event")
...
func test_build_slots_match_apartment_floor_visual_structure() -> void:
	...
	var roof_source := FileAccess.get_file_as_string("res://scripts/building/ApartmentRoof.gd")
	var floor_scene := FileAccess.get_file_as_string("res://scenes/building/Floor.tscn")
	...
	assert_true(building_scene.contains("[node name=\"ApartmentRoof\" type=\"Node2D\""), "ApartmentBuilding should preplace the single building roof node")
	assert_true(building_scene.contains("[node name=\"ApartmentTileMap\" parent=\"ApartmentRoof\""), "ApartmentRoof should reuse the editable apartment TileMap template")
	assert_true(roof_source.contains("roof_theme[\"offset_pixels\"]"), "ApartmentRoof should position from configured roof offset")
	assert_true(roof_source.contains("roof_theme[\"total_width_tiles\"]"), "ApartmentRoof should size from configured total roof width")
	...
	for floor in floors:
		...
		if floor_index > 1:
			assert_false(building_scene.contains("[node name=\"BuildSlot_%d\"" % floor_index), "ApartmentBuilding should not preplace whole-floor BuildSlot_%d" % floor_index)
	assert_true(floor_scene.contains("[node name=\"LeftBuildSlot\""), "Floor should preplace the left pending-room build slot")
	assert_true(floor_scene.contains("[node name=\"RightBuildSlot\""), "Floor should preplace the right pending-room build slot")
	...
	assert_true(slot_shell_scene.contains("BuildRoomShell"), "Build slots should draw one unbuilt room shell")
	assert_false(slot_shell_scene.contains("BuildServiceCore"), "Pending-room slots should not own the central elevator service area")
	assert_false(slot_shell_scene.contains("BuildRightRoomShell"), "Pending-room slots should not draw a second room shell")
	assert_true(slot_shell_scene.contains("name=\"BuildSlotShell\" type=\"Control\""), "Build slot shell should lay out a single room area in the editable scene")
	assert_true(slot_source.contains("room_edge_sides"), "BuildSlot should receive room edge ownership from Floor")
	assert_false(slot_source.contains("show_roof_eaves"), "BuildSlot should not carry per-room roof state")
	...
	assert_false(slot_shell_scene.contains("ServiceCoreTileMap.tscn"), "Pending-room slots should leave service core rendering to FloorServiceCore")
	...
	assert_true(slot_shell_scene.contains("StyleBoxTexture_status"), "Build slot action badge should use pixel UI button assets")
	assert_true(slot_shell_source.contains("var inset := 2.0"), "Build slot construction cover should leave only a 2px room-frame inset")
	...
	assert_true(slot_shell_scene.contains("text = \"300\""), "Build slot label should preview compact price copy in BuildSlotShell.tscn")
	assert_true(slot_shell_source.contains("false, false"), "BuildSlotShell should not enable roof or ApartmentTileMap construction marker tiles")
	assert_true(slot_shell_source.contains("_cover_rect"), "BuildSlotShell should size construction overlays from the whole room rect")
	...
	assert_false(building_source.contains("floor_index == highest_visible_floor"), "Apartment roof should not be split across top-floor room segments")
	assert_true(building_source.contains("highest_visible_floor <= 0"), "ApartmentBuilding should hide the single roof when no floor is visible")
