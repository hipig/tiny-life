...
公寓主体应改为 TileMap / TileMapLayer 优先的可编辑结构，并采用 scene-first / WYSIWYG authoring。`ApartmentBuilding.tscn` 预摆单个 `ApartmentRoof` 和所有 `Floor_X`；`Floor.tscn` 预摆左右房间、左右待建房间槽、左右公共区与中央服务核心；`Room.tscn` 预摆房间 shell、家具层、租客层和模板文本。脚本只绑定配置 ID、运行态数据和容器引用，刷新显隐/状态/文本；不从配置中的 scene path 动态选择核心楼层、房间或建造槽模板，也不运行时清空后重建核心结构。
...
3. ApartmentBuilding.tscn / Floor.tscn / Room.tscn / BuildSlot.tscn 保留可预览的固定节点树；房间墙体、地板和骨架改由 TileMapLayer 与拆分子场景承载；整栋公寓屋顶由 ApartmentBuilding 下的单个 ApartmentRoof 承载。
...
└── BuildSlotShell
    └── BuildRoomShell
        ├── ApartmentTileMap
        ├── ConstructionCover
        └── BuildSlotStatusPanel
```

`BuildSlot` 是单个待建房间槽，预摆在 `Floor.tscn` 的 `LeftBuildSlot` / `RightBuildSlot` 中，不再作为整层节点挂在 `ApartmentBuilding.tscn`。
...
```gdscript
func refresh_state():
    if not GameState.is_room_buildable(room_id):
        disabled = true
        return

    disabled = false
    shell.set_locked_visuals(false)
```
...
```gdscript
func confirm_build(room_id: String):
    var room_config = ConfigManager.get_room_config(room_id)
    var cost = room_config["build_cost"]
...
```text
公寓等级和低楼层完成度达到后，待建房间出现可施工状态
点击待建房间弹确认
金币不足确认按钮置灰
金币足够可建造
建造后该房间变为已建成，下一批满足条件的房间出现
```
