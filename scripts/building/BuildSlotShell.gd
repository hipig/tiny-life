...
func apply_layout(slot_size: Vector2, service_width: float, _wall_inset: float, _floor_height: float, _roof_height: float, frame_tiles := Vector2i(8, 4), tile_theme := {}) -> void:
	...
	if service_tile_map != null:
		...
		service_tile_map.render_room_skeleton(service_tiles, tile_theme, true, false)
	if apartment_tile_map != null:
		apartment_tile_map.render_room_skeleton(frame_tiles, tile_theme, true, true)
