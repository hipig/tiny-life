...
func open() -> void:
	...
	total_rent_value_template = _template_text("RentValueTemplate")
...
func _template_text(node_name: String) -> String:
	...
	if template_label == null:
		push_error("RentDetailPanel scene is missing TemplateText/%s." % node_name)
		return ""
